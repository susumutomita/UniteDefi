...
use near_sdk::{
    env, near_bindgen, AccountId, Balance, Gas, PanicOnDefault, Promise, PromiseResult, Timestamp,
};
...
impl FusionHTLC {
    ...
    fn execute_claim_transfers(&self, escrow_id: String, escrow: FusionEscrow) -> Promise {
        ...
        if let Some(token_id) = escrow.token_id {
            ...
            // NEAR transfers
            promise = Promise::new(escrow.beneficiary.clone()).transfer(escrow.amount);
            ...
            if escrow.safety_deposit > 0 {
                ...
                promise =
                    promise.then(Promise::new(safety_recipient).transfer(escrow.safety_deposit));
            }
        }
