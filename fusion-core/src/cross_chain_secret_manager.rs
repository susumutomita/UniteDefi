use crate::chains::ethereum::event_monitor::{EthereumEventMonitor, SecretRevealedEvent};
...
pub struct CrossChainSecretManager {
    ...
    eth_monitor: Option<EthereumEventMonitor>,
    near_monitor: Option<NearEventMonitor>,
}
...
impl CrossChainSecretManager {
    ...
    pub async fn start_monitoring(&self) -> Result<()> {
        ...
        if let Some(eth_monitor) = &self.eth_monitor {
            ...
            tokio::spawn(async move {
                eth_monitor.monitor_secret_revealed_events(eth_tx).await.ok();
            });
        ...
        if let Some(near_monitor) = &self.near_monitor {
            ...
            tokio::spawn(async move {
                near_monitor.monitor_secret_revealed_events(near_tx).await.ok();
            });
    ...
    pub async fn is_secret_revealed(&self, secret_hash: &SecretHash) -> bool {
        ...
        mappings.get(secret_hash)
            .map(|m| m.revealed_on.is_some())
            .unwrap_or(false)
    }
    
    pub async fn get_revelation_details(&self, secret_hash: &SecretHash) -> Result<Option<(String, u64)>> {
        let mappings = self.secret_mappings.read().await;
        
        if let Some(mapping) = mappings.get(secret_hash) {
            if let (Some(chain), Some(timestamp)) = (&mapping.revealed_on, mapping.revelation_timestamp) {
