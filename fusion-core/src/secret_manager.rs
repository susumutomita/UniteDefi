...
impl SecretManager {
    ...
    pub async fn process_claim_event(&mut self, event: &NearHtlcClaimEvent) -> Result<(), SecretError> {
        // シークレットのフォーマットを検証
        ...
        self.secrets.insert(event.escrow_id.clone(), event.secret.clone());
        Ok(())
    }
...
mod tests {
    ...
    async fn should_execute_cross_chain_claim_with_revealed_secret() {
        ...
        let result = cross_chain_executor.execute_claim(claim_request, &ethereum_connector).await;
    ...
    async fn should_fail_on_invalid_parameters() {
        ...
        let result = cross_chain_executor.execute_claim(invalid_request, &ethereum_connector).await;
