...
impl ClaimExecutor {
    ...
    pub async fn claim_on_ethereum(
        ...
        let eth_connector = self.eth_connector.as_ref()
            .ok_or_else(|| anyhow!("Ethereum connector not configured"))?;
        
        // Wait for secret to be revealed
        let secret = self.wait_for_secret_revelation(secret_hash, Duration::from_secs(3600)).await?;
        
        // Parse escrow address
        let escrow_address = escrow_address.parse::<Address>()
            .map_err(|e| anyhow!("Invalid escrow address: {}", e))?;
        
        // Claim the escrow
        let receipt = eth_connector.claim_escrow(escrow_address, secret).await?;
        
        Ok(format!("0x{:x}", receipt.transaction_hash))
    }
    
    /// Claim HTLC on NEAR using revealed secret
    pub async fn claim_on_near(
        &self,
        htlc_id: &str,
        secret_hash: &SecretHash,
    ) -> Result<String> {
        let near_connector = self.near_connector.as_ref()
    ...
    pub async fn get_claim_status(&self, secret_hash: &SecretHash) -> Result<ClaimStatus> {
        if let Some((chain, timestamp)) = self.secret_manager.get_revelation_details(secret_hash).await? {
            Ok(ClaimStatus {
