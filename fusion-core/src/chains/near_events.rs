// use borsh::{BorshDeserialize, BorshSerialize}; // TODO: Will be used for actual NEAR contract event parsing
...
impl NearEventParser {
    ...
    pub fn parse_create_event(log: &str) -> Result<NearHtlcCreateEvent, ParseError> {
        ...
        if parts.len() < 11 {
            return Err(ParseError::InvalidFormat(
                "Insufficient parts in log".to_string(),
            ));
        }
    ...
    pub fn parse_claim_event(log: &str) -> Result<NearHtlcClaimEvent, ParseError> {
        ...
        if parts.len() < 2 {
            return Err(ParseError::InvalidFormat(
                "Missing secret in log".to_string(),
            ));
        }
    ...
    /// Cancelイベントのパース
    pub fn parse_cancel_event(_log: &str) -> Result<NearHtlcCancelEvent, ParseError> {
        // TODO: Implement cancel event parsing
        Err(ParseError::InvalidFormat(
            "Cancel event parsing not implemented".to_string(),
        ))
    }
}
