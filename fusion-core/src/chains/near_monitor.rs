use crate::chains::near_events::{NearHtlcEvent, NearHtlcCreateEvent, NearHtlcClaimEvent, NearHtlcCancelEvent, NearEventParser};
use reqwest::Client;
...
impl NearRpcConnector {
    ...
    pub async fn get_contract_state_changes(&self, block_hash: &str, contract_id: &str) -> Result<Vec<String>, NearError> {
        let request = serde_json::json!({
            "jsonrpc": "2.0",
            "id": "dontcare",
...
impl NearHtlcMonitor {
    ...
    pub async fn start_monitoring(&self, sender: mpsc::Sender<NearHtlcEvent>) -> Result<(), NearError> {
        self.start_monitoring_with_config(sender, MonitorConfig::default()).await
    }
    ...
    async fn poll_for_events(&self) -> Result<Vec<NearHtlcEvent>, NearError> {
        let latest_block = self.rpc_connector.get_latest_block().await?;
...
mod tests {
    ...
    async fn should_monitor_contract_logs() {
        ...
        let monitor_task = tokio::spawn(async move {
            monitor.start_monitoring(tx).await
        });
    ...
    async fn should_retry_on_rpc_connection_failure() {
        let monitor = NearHtlcMonitor::new(
            "http://invalid-url.com", // 意図的に無効なURL
            "fusion_htlc.testnet",
        ).await;
        ...
        let monitor_task = tokio::spawn(async move {
            monitor.start_monitoring_with_config(tx, config).await
        });
