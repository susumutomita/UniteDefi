use super::limit_order_abi::LimitOrderProtocol;
...
impl OrderExtractor {
    ...
    pub async fn extract_order_by_hash(&self, order_hash: &str) -> Result<Order> {
        ...
        let hash_bytes =
            hex::decode(order_hash).map_err(|e| anyhow!("Failed to decode order hash: {}", e))?;
...
mod tests {
    ...
    fn test_invalid_address() {
        let result =
            OrderExtractor::new("https://base-sepolia.infura.io/v3/test", "invalid_address");
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Invalid limit order address"));
    }
}
