...
async fn should_extract_order_from_limit_order_protocol() {
    ...
    let limit_order_address = "0x31ad40b8aC12957bF0F956D5bA43Af6A730D7CB6"; // Base Sepolia deployment
    
    let extractor = OrderExtractor::new(rpc_url, limit_order_address)
        .expect("Should create extractor");
...
async fn should_handle_invalid_order_hash() {
    ...
    let rpc_url = "https://base-sepolia.infura.io/v3/test-key";
    let limit_order_address = "0x31ad40b8aC12957bF0F956D5bA43Af6A730D7CB6";
    ...
    assert!(result.unwrap_err().to_string().contains("Invalid order hash"));
}
