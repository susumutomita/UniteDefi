...
use fusion_core::chains::near::event_monitor::{HtlcEventType, NearEventMonitor};
use tokio::time::{sleep, Duration};

#[tokio::test]
async fn should_monitor_limit_order_events_on_ethereum() {
    // Skip test if no real API key is provided
    if std::env::var("ETHEREUM_WS_URL").is_err() {
        eprintln!(
            "Skipping should_monitor_limit_order_events_on_ethereum: ETHEREUM_WS_URL not set"
        );
        return;
    }

    // Given: An Ethereum event monitor connected to Base Sepolia
    let rpc_url = std::env::var("ETHEREUM_WS_URL").expect("ETHEREUM_WS_URL must be set");
...
async fn should_monitor_htlc_events_on_near() {
    ...
    let monitor = NearEventMonitor::new(near_rpc, htlc_contract).expect("Should create monitor");
    ...
    tokio::select! {
        Some(event) = rx.recv() => {
            ...
            match event.event_type {
                ...
                HtlcEventType::Claimed => assert!(event.secret.is_some()),
...
async fn should_detect_cross_chain_secret_revelation() {
    // Skip test if no real API key is provided
    if std::env::var("ETHEREUM_WS_URL").is_err() {
        eprintln!("Skipping should_detect_cross_chain_secret_revelation: ETHEREUM_WS_URL not set");
        return;
    }
    ...
    let eth_monitor =
        EthereumEventMonitor::new(&eth_ws_url, "0x171C87724E720F2806fc29a010a62897B30fdb62")
            .await
            .expect("Should create ETH monitor");

    let near_monitor = NearEventMonitor::new("https://rpc.testnet.near.org", "fusion-htlc.testnet")
