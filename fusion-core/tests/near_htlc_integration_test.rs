...
fn create_test_order() -> Order {
    Order {
        ...
        making_amount: 1000000000000000000000000, // 1 NEAR
        taking_amount: 5000000,                   // 5 USDC
        allowed_sender: "0x0000000000000000000000000000000000000000".to_string(),
        offsets: 0,
        interactions: "0x".to_string(), // In real implementation, this would contain HTLC data
    }
}
...
async fn should_complete_full_cross_chain_swap_flow() {
    ...
    let claim_event = create_mock_htlc_claim_event("fusion_0".to_string(), hex::encode(secret));
...
async fn test_monitor_with_event_processing() {
    ...
    let _timeout_result = tokio::time::timeout(Duration::from_millis(100), event_rx.recv()).await;
}
